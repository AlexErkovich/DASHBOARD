...
<body>
    ...
    <script>
        ...
        function validateForm() {
            ...
            // Перебираем каждый блок и удаляем пустые поля
            let blocks = form.querySelectorAll('.todo__block');
            blocks.forEach((block) => {
                let inputs = block.querySelectorAll('input');
                inputs.forEach((input) => {
                    if (isEmpty(input)) {
                        input.remove(); // Удаляем пустое поле
                    }
                });
            });

            // Проверка наличия непустых полей
            let nonEmptyInputs = form.querySelectorAll('input[name^="communication"], input[name^="advertisement"], input[name^="production"], input[name^="finance"], input[name^="quality"]');
